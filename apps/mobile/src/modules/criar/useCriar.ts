...
function mapPlaneta(p: any, membros: MembroPlaneta[], userId: string): Planeta {
  ...
  return {
    ...
    isEstacao: false, estacaoAtiva: false,
...
export function useCriar() {
  ...
  const planetasQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("planetas").select("*").eq("is_estacao", false).order("created_at");
      ...
      return { planetas: todos, convites };
    },
  });
  ...
  async function uploadToBucket(bucket: string, planetaId: string, uri: string, fileName: string, mimeType: string): Promise<string> {
    const ext = fileName.split(".").pop() || "bin";
    const path = `${planetaId}/${userId}/${Date.now()}-${Math.round(Math.random() * 1e6)}.${ext}`;
    const response = await fetch(uri);
    const blob = await response.blob();
    const { error } = await supabase.storage.from(bucket).upload(path, blob, { upsert: true, contentType: mimeType });
    if (error) throw error;
    return path;
  }

  const addRecurso = useMutation({
    mutationFn: async (input: { planetaId: string; uri: string; fileName: string; mimeType: string }) => {
      const url = await uploadToBucket("planeta-recursos", input.planetaId, input.uri, input.fileName, input.mimeType);
      const ext = input.fileName.split(".").pop()?.toUpperCase() || "Arquivo";
      const { error } = await supabase.from("recursos").insert({
        planeta_id: input.planetaId, autor_id: userId, nome: input.fileName, arquivo_url: url, tipo: ext,
      });
      if (error) throw error;
    },
  ...
  const addFoto = useMutation({
    mutationFn: async (input: { planetaId: string; uri: string; fileName: string; mimeType: string }) => {
      const url = await uploadToBucket("planeta-fotos", input.planetaId, input.uri, input.fileName, input.mimeType);
  ...
  return {
    ...
    addPlaneta, deletePlaneta, addRelatorio, addRecurso, addFoto, addEvento, attachProof, desativarEstacao,

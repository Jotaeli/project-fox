...
export function NovaSecaoModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  function submit() {
    if (!nome.trim()) {
      Alert.alert("Dê um nome pra seção.");
      return;
    }
    addSecao.mutate({ nome: nome.trim(), cor: hue }, { onSuccess: () => { setNome(""); onClose(); } });
  }
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={styles.card} onPress={(e) => e.stopPropagation()}>
          <Text style={typography.subtitle}>Nova seção</Text>
            ...
            <View style={styles.swatches}>
              {HUES.map((h) => (
                <Pressable
                  key={h}
                  onPress={() => setHue(h)}
                  style={[styles.swatch, { backgroundColor: `hsl(${h}, 55%, 45%)` }, hue === h && styles.swatchSel]}
                />
              ))}
            </View>
          </View>

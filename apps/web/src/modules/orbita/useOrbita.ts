...
export interface NotificationItem {
  id: string;
  atorId: string | null;
  tipo: "amizade_pedido" | "amizade_aceita" | "cutucada" | "reacao" | "desafio_convite" | "desafio_aceito" | "desafio_concluido" | "planeta_convite" | "planeta_aceito" | "espaco_notas_convite" | "espaco_notas_aceito";
...
export function useOrbita(userId?: string) {
  ...
  const uploadAvatar = useMutation({
    mutationFn: async (file: File) => {
      ...
      if (!byMime[file.type]) throw new Error("Use uma imagem JPG, PNG, WebP ou GIF.");
      if (file.size > 5 * 1024 * 1024) throw new Error("A foto precisa ter no máximo 5 MB.");
      const ext = byMime[file.type];
      const path = `${userId}/avatar.${ext}`;
      const { error } = await supabase.storage.from("avatares").upload(path, file, { upsert: true, contentType: file.type, cacheControl: "3600" });
      if (error) throw error;
